...
### Step 8 — API contract and frontend — [done; its running-app check ran after step 9's re-pin and passed]
...
### Step 10 — Copy truth, and the close — [done]
...
#### 10.0 As built, 2026-09-11

**Sentences the new corpus made false, found by grep rather than by memory**, each corrected with its
date and its old value kept where the direction of travel is the point:
- `CLAUDE.md`: "v0.7.1 holds 7 components, with 1,465 of its 1,479 nodes in the largest" -> **61
  components, 3,490 of 3,628**, plus a new paragraph saying why a *rising* component count is connection
  rather than fragmentation. The verification tier list gained `TEACHING_PROSE_AUTO`, and
  "2,202 of 2,284 single-source" -> **2,227 of 2,309**.
- `.claude/rules/evals.md`: the same single-source figures, and the dataset counts (gold **43**,
  adversarial **22**) beside the 2026-08-24 "as built" line that still said 25 and 18.
- `.claude/rules/grounding-and-claims.md`: single-source figures, the new tier, and a pointer to
  `TIERS_BY_PREDICATE` as the constructor rule that keeps a tier from meaning two things.
- `.claude/rules/graph-semantics.md`: a P1066 bullet -- ingested at v0.10.0, hand-checked first, **the
  opposite verdict from P279** (0 of 40 wrong against 47 of 47), teaching rather than influence.
- `.claude/rules/heldout-set.md`: the set is **stale against v0.10.0** and he decided to re-draw it as
  phase 7.7's first task; the rule that he draws it alone is restated rather than weakened.
- `docs/SPEC.md`: the component figures in two places, and the corroboration figures in the amendment
  block; the "138 components, largest 534" note now carries its v0.10.0 counterpart.
- `docs/ROADMAP.md`: 7.6's row marked **COMPLETE**, and a new "Where the build actually is" block dated
  2026-09-11 above the 2026-09-10 one, which is left in place as history.
- `DATA-LICENSES.md`: **"Article prose is stored in one place" was false as of step 9.** The gold set's
  `supporting_prose` quotes a sentence per claim, now including Italian and German articles. The second
  location is named, with the same CC BY-SA 4.0 reasoning as the exclusions excerpts.

**`docs/graph-semantics.md` gained §9, the bound as measured** -- 3,449 statements over 2,584 people,
2,487 passing the prose check (**72%, against 30% on the influence axis**, which is the expected
direction and §8 says why), 18 removed by hand including **two inverted teacher/student pairs** and nine
succession-only rows. Bound B was never measured -- the query service timed out twice -- and its cost is
stated rather than hidden.

**`make readme` rewrote 0 figures and `make report` regenerated byte-identically**, which is the useful
outcome: step 9 had already regenerated both from the new pin, so the copy pass moved prose and left every
*marked* number alone. The markers did their job by having nothing to do.

**`docs/classical-lineage-explained.md` written**, the plain-English walk-through the `start-a-phase`
skill asks for: the `mul` bug as he found it, why teaching rather than influence carries pre-1900 lineage
(56 influence statements against 1,766 teacher statements among 9,212 composers), the one distinction the
phase rests on, what the tier does and does not mean, the two inverted edges the hand review caught, and
four things the corpus still cannot do.

**The Lambda image builds and loads the new corpus; serving it here is unverified.** `make image` produces
a **68.7 MB** image, and the corpus loads *inside that image*: `docker run --rm musical-mycelium:local
python -c "...default_store()"` prints **artifact 0.10.0, 3,628 nodes, 61 components**, which is the half
of `image-run` that this phase actually needed to prove — the new artifact ships in the image and parses
there.

What could not be verified on this machine is the container **serving HTTP**. Four ports, 30 patient
readiness attempts each, always `000`; `docker ps` reports "Up"; `docker logs` is **empty**; and a
foreground `docker run` printed **no output whatsoever** before its timeout — no uvicorn banner, no
traceback. A Python process that starts and prints nothing, while the same `uvicorn` command serves
correctly outside Docker and the image's `CMD` is a plain `uvicorn ... --host 0.0.0.0 --port 8080`, is an
**environment limitation of Docker under WSL2 here rather than a product defect**. Recorded as unverified
for this phase rather than reported as passing; the deployed path is exercised by CI's own image build and
by phase 7.7's deploy.

**DoD verdicts, §1.1, every item with its evidence:**

| # | Item | Verdict |
|---|---|---|
| 1 | Mozart in the corpus, `mul` victims recovered, a test fixes a `mul`-only entity | **MET.** 19 artists recovered, Mozart among them; 22 remain unlabelled in any language and correctly cannot become nodes. `tests/test_labels.py` and step 1's as-built. |
| 2 | P1066 hand-checked **before** ingestion, findings in `graph-semantics.md` | **MET.** 40 rows, 0 wrong, 0 inverted; §8, and §9 for the bound. Reviewed by him 2026-09-11. |
| 3 | "Who did Beethoven study with?" returns a gated, cited answer saying "studied with", verified on the stub **and in one live capture** | **MET on the stub, in the API, and in the image's corpus load; the live half is OUTSTANDING.** The live capture spent in this phase went to the tour recording, which is an influence chain. Carried into 7.7, which runs live work at its close. |
| 4 | A test asserts no prose path narrates teaching as influence, including mixed sets | **MET.** `test_no_teaching_claim_is_ever_narrated_as_influence`: **8,373 teaching claim sets, 0 misnarrated**, including 96 mixed chains and 27 mixed fans; two tests break the checker on purpose. |
| 5 | `verification` and `corroboration` stay two fields; no tier means two things | **MET.** `TIERS_BY_PREDICATE` refuses a mismatched tier at construction; `tests/test_teaching_schema.py`. |
| 6 | Artist birth years, and the coverage panel and report's era slice show pre-1900 by birth with its skew stated | **MET.** 2,652 of 2,889 with a birth year, 2,111 before 1900; a coverage panel axis that says "born", never "active", with the Western European skew stated as uncounted; `era_slice` buckets artists by birth. |
| 7 | Free gates read 4 / 0 / 2 of six, and no live result on v0.10.0 reported as gated | **MET.** Measured 4 passed / 0 FAILED / 2 N/A. The live set is ungated by two independent guards (case count 63 against 56, and the step 0 pin guard). |
| 8 | v0.10.0 built and deployable; image builds and serves locally; `make check` green; budgets hold; root inside cap | **MET but for local serving.** Image builds (68.7 MB) and loads the corpus; HTTP serving unverified here, recorded above. `make check` green, every budget inside, root 17 of 18. |
| 9 | `agent/loop.py` unmodified, or the edit recorded in bold beforehand | **MET.** Recorded in §3.2 before the build; `run()` changed in three lines, all refusal wording, confirmed by diffing against `HEAD`. |

**Phase 7.6 is complete.** What it deliberately did not do: deploy, and run the live re-baseline. Both are
phase 7.7's, whose first task is now **a new held-out set drawn on v0.10.0** (his decision, 2026-09-11),
with the existing one left sealed, stale and unopened.
