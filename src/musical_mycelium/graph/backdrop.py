...
def build() -> Packed:
    ...
    links = [
        ...
        for e in edges
        if e["subject_id"] in index and e["object_id"] in index
    ]

    px, py = solve(len(keep), links)
...
//
// {nodes} nodes, {edges} edges, seed {seed}. `tests/test_backdrop.py` re-solves from the pinned
