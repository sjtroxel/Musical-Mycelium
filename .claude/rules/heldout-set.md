...
- **Never decrypt `eval/datasets/heldout_v2.json.enc`, never read a decrypted copy, and never ask for the
...
- **`make heldout-verify` and `make heldout-check` are safe to run and safe to read.** `verify` needs no
  ...
  (`heldout_v2_007: claims-diverged`) and never case content. A case id is not content. Use these instead
  of opening anything.
...
- **A skipped `test_the_committed_sealed_set_matches_its_manifest` means the set does not exist yet.**
  *(It skipped again on 2026-09-12, for a few minutes between the `heldout_v2` rename and the seal, and
...
- **THE CURRENT SET HAS NEVER BEEN RUN. `heldout_v2`, run count 0 — 2026-09-12.** ~~THE SET HAS BEEN RUN.
  ...
  say so and let him decide, rather than making the change and re-running.

- **THE RE-DRAW IS DONE — 2026-09-12, phase 7.7 step 0. `heldout_v2` exists, sealed, pinned 0.10.0.**
  ~~IT IS STALE AGAINST v0.10.0, AND HE DECIDED TO RE-DRAW IT — 2026-09-11, phase 7.6 step 10. A fresh
  draw is the FIRST task of phase 7.7.~~ *(Struck 2026-09-12. The sentence was an **instruction**, and an
  instruction outlives the task it names — which is the failure mode this project has already recorded
  once, with the "169 components" line in `CLAUDE.md`.)* He drew it from his own seed, sealed it, shredded
  the plaintext and checked it: **10 cases, 2 refusals, shapes descendants 2 / origins 5 / path 2 /
  teachers 1** (`origins` reads 5 because the two refusal cases are origins questions),
  `make heldout-verify` matches its manifest, `make heldout-check` reports **zero findings** against
  artifact 0.10.0. No agent read a case, drafted one, or learned the seed. **Run count 0.** The rules
  above are unchanged by any of it.
