...
.DEFAULT_GOAL := help
.PHONY: help install fmt lint typecheck test cov check root-check orient clean dev ingest backdrop \
...
backdrop: ## Regenerate the SPA's inlined backdrop layout from the pinned artifact (free, offline)
	uv run python -m musical_mycelium.graph.backdrop
...
heldout-draw: ## Draw a held-out set. Usage: make heldout-draw SEED='...' OUT=~/heldout_vN.json
	@test -n "$(SEED)" || { echo "Usage: make heldout-draw SEED='something only you know' OUT=~/heldout_vN.json"; exit 1; }
	@test -n "$(OUT)" || { echo "Usage: make heldout-draw SEED='something only you know' OUT=~/heldout_vN.json"; exit 1; }
	uv run python -m musical_mycelium.eval.heldout_draw --seed "$(SEED)" --out "$(OUT)"

heldout-seal: ## Seal an authored held-out set. Usage: make heldout-seal PLAINTEXT=~/path/heldout_vN.json
	@test -n "$(PLAINTEXT)" || { echo "Usage: make heldout-seal PLAINTEXT=~/path/heldout_vN.json"; exit 1; }
