...
``test_a_third_tool_does_not_touch_the_loop`` is **invariant 4**. If it ever needs editing to
accommodate a new tool, the seam is broken.
...
def test_the_v01_tools_satisfy_the_protocol(store: InMemoryGraphStore) -> None:
...
def test_tool_config_is_the_bedrock_shape(registry: ToolRegistry) -> None:
    ...
    assert {s["toolSpec"]["name"] for s in specs} == {"resolve_genre", "get_influences"}
    for spec in specs:
        assert spec["toolSpec"]["description"]
        assert spec["toolSpec"]["inputSchema"]["json"]["type"] == "object"


def test_a_third_tool_does_not_touch_the_loop(store: InMemoryGraphStore) -> None:
